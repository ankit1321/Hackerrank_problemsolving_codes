...
void quicksort(int* prices,int first,int last)
{
    int pivot,j,temp,i;
     ...
     if(first<last)
     {
         ...
         while(i<j)
         {
             while(prices[i]<=prices[pivot]&&i<last)
                 i++;
         ...
         temp=prices[pivot];
         prices[pivot]=prices[j];
         prices[j]=temp;

         quicksort(prices,first,j-1);
         quicksort(prices,j+1,last);
